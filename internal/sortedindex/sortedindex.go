...
// KeyFunc extracts a source line's index key — see kvtypes.KeyFunc.
type KeyFunc = kvtypes.KeyFunc
...
type SortedIndex struct {
	...
	// read, and any read that loses the race with Close observes kvtypes.ErrClosed
...
func verifySidxHeader(f *os.File) (int64, error) {
	...
	version, err := kvtypes.ReadUint32(r)
...
func readSparseFile(path string) ([]sparseEntry, error) {
	...
	for range count {
		keyLen, err := kvtypes.ReadUint16(r)
		if err != nil {
			return nil, err
		}
...
func readSidxEntry(r *bufio.Reader) (sidxEntry, error) {
	...
	fileIdx, err := kvtypes.ReadUint16(r)
	if err != nil {
		return sidxEntry{}, err
	}
	srcOff, err := kvtypes.ReadUint64(r)
	if err != nil {
		return sidxEntry{}, err
	}
	length, err := kvtypes.ReadUint32(r)
	if err != nil {
		return sidxEntry{}, err
	}
...
// second call, or any Get/Prefix/All call after Close, returns kvtypes.ErrClosed.
...
// Get returns the current line for key, or kvtypes.ErrNotFound — same contract as
...
func (si *SortedIndex) Get(key []byte) ([]byte, error) {
	...
	for {
		...
		if cmp > 0 {
			return nil, kvtypes.ErrNotFound // sorted order: key is absent
		}
	}
}
...
func (si *SortedIndex) Has(key []byte) (bool, error) {
	_, err := si.Get(key)
	if errors.Is(err, kvtypes.ErrNotFound) {
		return false, nil
	}
	return err == nil, err
}
