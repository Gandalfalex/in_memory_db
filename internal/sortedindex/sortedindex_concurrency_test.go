...
func TestBuildSortedIndexConcurrentCallsSameSidxPathDoNotCorrupt(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		errs[0] = BuildSortedIndex(context.Background(), []string{pathA}, fileindex.JSONStringKey("id"), sidxPath, opts)
	}()
	go func() {
		defer wg.Done()
		errs[1] = BuildSortedIndex(context.Background(), []string{pathB}, fileindex.JSONStringKey("id"), sidxPath, opts)
	}()
	wg.Wait()
...
func TestEnsureFreshConcurrentCallsSameSidxPathDoNotCorrupt(t *testing.T) {
	...
	for i := range 4 {
		go func(i int) {
			defer wg.Done()
			results[i], errs[i] = EnsureFresh(context.Background(), []string{srcPath}, sidxPath, fileindex.JSONStringKey("id"), opts)
		}(i)
	}
	wg.Wait()
...
func TestBuildSortedIndexRespectsAlreadyCancelledContext(t *testing.T) {
	...
	err := BuildSortedIndex(ctx, []string{srcPath}, fileindex.JSONStringKey("id"), sidxPath, SortedIndexOptions{ChunkEntries: 2, SparseInterval: 2})
...
func TestBuildSortedIndexCancelMidScan(t *testing.T) {
	...
	err := BuildSortedIndex(ctx, []string{srcPath}, fileindex.JSONStringKey("id"), sidxPath, SortedIndexOptions{ChunkEntries: 10_000, SparseInterval: 4096})
	if !errors.Is(err, context.Canceled) {
