...
func TestFileIndexPutGetRoundtrip(t *testing.T) {
	...
	if _, err := fi.Get([]byte("c")); !errors.Is(err, kvtypes.ErrNotFound) {
...
func TestFileIndexDeleteIsIndexOnly(t *testing.T) {
	...
	if _, err := fi.Get([]byte("a")); !errors.Is(err, kvtypes.ErrNotFound) {
...
func TestFileIndexPutValidation(t *testing.T) {
	...
	if err := fi.Put([]byte("nokeyhere")); !errors.Is(err, kvtypes.ErrNoLineKey) {
		t.Fatalf("Put(no key) = %v, want ErrNoLineKey", err)
	}
	if err := fi.Put([]byte("a,em\nbedded")); !errors.Is(err, kvtypes.ErrInvalidLine) {
...
func TestFileIndexClosed(t *testing.T) {
	...
	if err := fi.Close(); !errors.Is(err, kvtypes.ErrClosed) {
		t.Fatalf("second Close = %v, want ErrClosed", err)
	}
	if _, err := fi.Get([]byte("a")); !errors.Is(err, kvtypes.ErrClosed) {
		t.Fatalf("Get after Close = %v, want ErrClosed", err)
	}
	if err := fi.Put([]byte("b,2")); !errors.Is(err, kvtypes.ErrClosed) {
		t.Fatalf("Put after Close = %v, want ErrClosed", err)
	}
	if err := fi.Sync(); !errors.Is(err, kvtypes.ErrClosed) {
...
func TestFileIndexIterator(t *testing.T) {
	...
	var keys []string
	for k, v := range fi.All(kvtypes.IterOptions{Sorted: true}) {
	...
	keys = keys[:0]
	for k := range fi.All(kvtypes.IterOptions{Prefix: []byte("a"), Sorted: true, Reverse: true}) {
		keys = append(keys, string(k))
	}
	...
	// DB.Iterator.
	it := fi.Iterator(kvtypes.IterOptions{Sorted: true})
	defer it.Close()
	if err := fi.Delete([]byte("a")); err != nil {
		t.Fatal(err)
	}
