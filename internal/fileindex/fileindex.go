// Package fileindex is the plain-JSONL sibling of internal/bitcask: see
// FileIndex's doc comment.
package fileindex
...
func (fi *FileIndex) Put(line []byte) error {
	...
	if !ok || len(key) == 0 {
		return kvtypes.ErrNoLineKey
	}
	if bytes.IndexByte(line, '\n') >= 0 {
		return kvtypes.ErrInvalidLine
	}
...
func (fi *FileIndex) Iterator(opts kvtypes.IterOptions) *kvtypes.Iterator {
	fi.mu.RLock()
	...
	return kvtypes.NewIterator(keys, opts, fi.Get)
}
...
func (fi *FileIndex) All(opts kvtypes.IterOptions) iter.Seq2[[]byte, []byte] {
