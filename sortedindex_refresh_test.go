...
func TestEnsureFreshIncrementalRefreshAppendsNewSource(t *testing.T) {
	...
	si1, err := EnsureFresh(context.Background(), []string{basePath}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
		t.Fatal(err)
	}
	si1.Close()
	...
	si2, err := EnsureFresh(context.Background(), []string{basePath, changesPath}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
...
func TestRefreshSortedIndexNeverReopensExistingSources(t *testing.T) {
	...
	sidxPath := filepath.Join(dir, "index.sidx")
	opts := SortedIndexOptions{ChunkEntries: 2, SparseInterval: 2}
	if err := BuildSortedIndex(context.Background(), []string{basePath}, JSONStringKey("id"), sidxPath, opts); err != nil {
		t.Fatal(err)
	}
	...
	if err := refreshSortedIndexLocked(context.Background(), recorded, newSrcs, sidxPath, JSONStringKey("id"), opts); err != nil {
...
func TestEnsureFreshIncrementalRefreshMultipleNewSources(t *testing.T) {
	...
	si2, err := EnsureFresh(context.Background(), []string{basePath, change1, change2}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
...
func TestEnsureFreshIncrementalRefreshRespectsCancelledContext(t *testing.T) {
	...
	cancel()
	_, err = EnsureFresh(ctx, []string{basePath, changesPath}, sidxPath, JSONStringKey("id"), opts)
