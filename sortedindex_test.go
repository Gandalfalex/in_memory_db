...
func buildTestSortedIndex(t *testing.T, lines []string, opts SortedIndexOptions) (*SortedIndex, string) {
	...
	if err := BuildSortedIndex(context.Background(), []string{srcPath}, JSONStringKey("id"), sidxPath, opts); err != nil {
...
func TestSortedIndexGetAndLastLineWins(t *testing.T) {
	...
	if _, err := si.Get([]byte("nope")); !errors.Is(err, ErrNotFound) {
		t.Fatalf("Get(nope) = %v, want ErrNotFound", err)
	}

	if _, err := si.Get(nil); !errors.Is(err, ErrEmptyKey) {
...
func TestSortedIndexBloomRejectsAbsentKeys(t *testing.T) {
	...
	if _, err := si.Get([]byte("definitely-absent")); !errors.Is(err, ErrNotFound) {
...
func TestSortedIndexEmptyIndex(t *testing.T) {
	...
	if _, err := si.Get([]byte("anything")); !errors.Is(err, ErrNotFound) {
...
func TestSortedIndexReopenReadsSidecars(t *testing.T) {
	...
	reopened, err := OpenSortedIndex(sidxPath, JSONStringKey("id"))
	if err != nil {
...
func TestSortedIndexHas(t *testing.T) {
	...
	if _, err := si.Has(nil); !errors.Is(err, ErrEmptyKey) {
...
func TestSortedIndexHasAfterClose(t *testing.T) {
	...
	si.Close()
	if _, err := si.Has([]byte("a")); !errors.Is(err, ErrClosed) {
...
func TestSortedIndexMultiSourceLaterFileWins(t *testing.T) {
	...
	if err := BuildSortedIndex(context.Background(), []string{basePath, changesPath}, JSONStringKey("id"), sidxPath, opts); err != nil {
		t.Fatalf("BuildSortedIndex: %v", err)
	}
	si, err := OpenSortedIndex(sidxPath, JSONStringKey("id"))
	if err != nil {
...
func TestSortedIndexThreeSourcePrecedenceChain(t *testing.T) {
	...
	opts := SortedIndexOptions{ChunkEntries: 2, SparseInterval: 2}
	if err := BuildSortedIndex(context.Background(), []string{basePath, change1Path, change2Path}, JSONStringKey("id"), sidxPath, opts); err != nil {
		t.Fatal(err)
	}
	si, err := OpenSortedIndex(sidxPath, JSONStringKey("id"))
	if err != nil {
...
func TestSortedIndexOpenClosesAlreadyOpenedSourcesOnFailure(t *testing.T) {
	...
	if _, err := OpenSortedIndex(sidxPath, JSONStringKey("id")); err == nil {
...
func TestEnsureFreshBuildsOnceThenReusesCache(t *testing.T) {
	...
	si1, err := EnsureFresh(context.Background(), []string{srcPath}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
	...
	si2, err := EnsureFresh(context.Background(), []string{srcPath}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
...
func TestEnsureFreshErrorsClearlyWhenSourceRemoved(t *testing.T) {
	...
	sidxPath := filepath.Join(dir, "index.sidx")
	opts := SortedIndexOptions{ChunkEntries: 2, SparseInterval: 2}
	si, err := EnsureFresh(context.Background(), []string{basePath, changesPath}, sidxPath, JSONStringKey("id"), opts)
	if err != nil {
		t.Fatal(err)
	}
	si.Close()
	...
	_, err = EnsureFresh(context.Background(), []string{basePath, changesPath}, sidxPath, JSONStringKey("id"), opts)
	if err == nil {
